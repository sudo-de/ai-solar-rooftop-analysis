...
class SolarAnalysisEngine:
    ...
    def analyze_rooftops(
        self, image_paths: List[str], cities: List[str], panel_types: List[str]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            panel_types = (
                panel_types + [panel_types[-1]] if panel_types else ["monocrystalline"]
            )[:n]

            # Process each rooftop
            for i, (image_path, city, panel_type) in enumerate(
                zip(image_paths, cities, panel_types)
            ):
                try:
                    result = self._analyze_single_rooftop(
                        image_path, city, panel_type, i + 1
                    )
                    results.append(result)
                except Exception as e:
                    self.logger.error(f"Failed to analyze rooftop {i+1}: {e}")
                    results.append(
                        {"rooftop_id": i + 1, "error": str(e), "status": "failed"}
                    )

            # Generate summary
            successful_analyses = [r for r in results if "error" not in r]
            total_energy = sum(
                r.get("annual_energy_kwh", 0) for r in successful_analyses
            )
            total_savings = sum(
                r.get("roi_estimation", {}).get("annual_savings", 0)
            ...
            return {
                ...
                "results": results,
                "summary": {
                    "average_energy_per_rooftop": (
                        total_energy / len(successful_analyses)
                        if successful_analyses
                        else 0
                    ),
                    "average_savings_per_rooftop": (
                        total_savings / len(successful_analyses)
                        if successful_analyses
                        else 0
                    ),
                    "success_rate": (
                        len(successful_analyses) / len(results) if results else 0
                    ),
                },
            }
    ...
    def _analyze_single_rooftop(
        self, image_path: str, city: str, panel_type: str, rooftop_id: int
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                cad_model = self.analyzer_3d.analyze_roof_3d(
                    image_path, (lat, lon), panel_type
                )
                cad_analysis = {
    ...
    def _analyze_image(self, image_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            segmented_image_base64 = (
                self._image_to_base64(segmented_image_path)
    ...
    def _calculate_solar_potential(
        self,
        area_m2: float,
        orientation: str,
        lat: float,
        ...
        try:
            ...
            panel_efficiency = self.solar_constants["panel_types"][panel_type][
                "efficiency"
            ]
            ...
            annual_energy = (
                area_m2 * panel_efficiency * peak_sun_hours * 365 * orientation_factor
            )
    ...
    def _estimate_roi(
        self, energy_kwh: float, city: str, panel_type: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            system_size_kw = min(
                max(system_size_kw, 0.1), 5.0
            )  # Cap between 0.1-5.0 kW

            # Calculate costs
            total_cost = (
                (system_size_kw * 1000 * panel_constants["cost_per_watt"])
            ...
            monthly_savings = [
                e * self.solar_constants["electricity_rate"] for e in monthly_energy
            ]

            # Calculate payback period
            payback_period = (
                total_cost / annual_savings if annual_savings > 0 else float("inf")
            )
            payback_period = max(
                4.0, min(20.0, payback_period)
            )  # Cap between 4-20 years
    ...
    def _generate_recommendations(
        ...
        if suitability >= 7:
            recommendations.append(
                f"Highly suitable for solar installation ({surface_type} rooftop)"
            )
        elif suitability >= 4:
            recommendations.append(
                f"Moderately suitable for solar installation ({surface_type} rooftop)"
            )
        else:
            recommendations.append(
                f"Limited suitability - consider alternatives ({surface_type} rooftop)"
            )

        # Panel type recommendations
        panel_constants = self.solar_constants["panel_types"][panel_type]
        recommendations.append(
            f"Recommended panel type: {panel_type} ({panel_constants['efficiency']*100:.0f}% efficiency)"
        )
        ...
        if orientation.lower() != "south":
            recommendations.append(
                f"Optimize panel tilt for {orientation} orientation (15-30°)"
            )

        # Surface type recommendations
        if surface_type.lower() == "sloped":
            recommendations.append(
                "Ensure structural integrity for sloped installation"
            )
    ...
    def _create_segmentation_visualization(
        self, image_path: str, segmentation_data: list
    ) -> str:
